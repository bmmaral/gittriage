...
fn load_nexusignore(root: &Path) -> Vec<glob::Pattern> {
    let path = root.join(".nexusignore");
...
fn is_nexusignored(path: &Path, patterns: &[glob::Pattern]) -> bool {
...
pub fn scan_roots(roots: &[PathBuf], options: &ScanOptions) -> Result<Vec<CloneRecord>> {
    ...
    for root in roots {
        let ignore_patterns = load_nexusignore(root);
        ...
        for entry in walker.build() {
            ...
            if is_nexusignored(path, &ignore_patterns) {
                continue;
            }
...
mod tests {
    ...
    #[test]
    fn nexusignore_excludes_matching_paths() {
        let tmp = TempDir::new().unwrap();
        let root = tmp.path();
        ...
        fs::write(root.join(".nexusignore"), "skip-me\n").unwrap();
        ...
        assert!(
            ...
            ".nexusignore should exclude matching dirs"
        );
    }
