use chrono::Utc;
use nexus_core::{CloneRecord, InventorySnapshot};
use nexus_plan::{build_plan_with, PlanBuildOpts};
use std::fs;
use std::path::Path;
...
fn git(dir: &Path, args: &[&str]) {
    let status = Command::new("git")
        ...
        .env("GIT_AUTHOR_EMAIL", "nexus@example.com")
        .env("GIT_COMMITTER_NAME", "nexus")
        .env("GIT_COMMITTER_EMAIL", "nexus@example.com")
        .status()
...
fn merge_base_evidence_is_attached_best_effort() {
    ...
    let repo_a = base.join(format!("nexus-mb-a-{}", Uuid::new_v4()));
    let repo_b = base.join(format!("nexus-mb-b-{}", Uuid::new_v4()));
